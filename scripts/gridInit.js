...
function initializeGrid(width, height, seed = '', mode = 'random') {
  const cells = [];
  
  if (mode === 'classic') {
    // Classic mode: left half black, right half white
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        cells.push({ 
          x, 
          y, 
          color: x < midX ? 'black' : 'white'
        });
      }
    }
  } else {
    // Random mode: seeded random distribution
...
function initializeBalls(
  ...
  if (mode === 'classic') {
    // Classic mode: place balls randomly on their respective sides
    // Place black balls randomly on white (right) side
    ...
    // Place white balls randomly on black (left) side
    ...
    // Random mode: place balls randomly on appropriate colored squares
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { initializeGrid, initializeBalls, seededRandom, Grid };
}
